...
#define LEVIATHAN_CLEAR_BIT(value, bit) (value &= static_cast<decltype(value)>(~(static_cast<decltype(value)>(1) << bit)))

// Sets bit bit of value.
#define LEVIATHAN_SET_BIT(value, bit) (value |= static_cast<decltype(value)>(static_cast<decltype(value)>(1) << bit))

// Toggles bit bit of value.
#define LEVIATHAN_TOGGLE_BIT(value, bit) (value ^= static_cast<decltype(value)>(static_cast<decltype(value)>(1) << bit))

// Checks if bit bit of value is set. Macro returns the value of bit bit (0 if cleared and 1 if set).
#define LEVIATHAN_CHECK_BIT(value, bit) ((value >> bit) & 1)

// Declares bitwise operators for the enum type.
#define LEVIATHAN_DECLARE_ENUM_BIT_OPERATORS(EnumType)																												\
